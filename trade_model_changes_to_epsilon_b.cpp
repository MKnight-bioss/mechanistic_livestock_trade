...
int main() {
    ...
    int num_params = 11; //number of parameters to run
    ...
    double epsilon_b = 1.0; //parameter scaling propensity to trade with trade partners
    ...
    for (int np = 0; np < num_params; ++np) {
        ...
        //values of epsilon_b to consider
        if (np == 0) epsilon_b = 0.00001;
        else if (np == 1) epsilon_b = 0.0001;
        else if (np == 2) epsilon_b = 0.001;
        else if (np == 3) epsilon_b = 0.01;
        else if (np == 4) epsilon_b = 0.1;
        else if (np == 5) epsilon_b = 1.0;
        else if (np == 6) epsilon_b = 10.0;
        else if (np == 7) epsilon_b = 100.0;
        else if (np == 8) epsilon_b = 1000.0;
        else if (np == 9) epsilon_b = 10000.0;
        else epsilon_b = 100000.0;
        ...
        for (int nr = 0; nr < num_reps; ++nr) {
            ...
            int trade_change_introduced = 0; //variable that takes value 1 when changes to trade are made
            ...
            while (dt < t_max) {
                ...
                //check to see if it's time to change trade (assuming epsilon != 1)
                if (dt >= trade_change_time) {
                    if (trade_change_introduced == 0) {
                        trade_change_introduced = 1;
                        if (epsilon_b != 1.0) {
                            //update trade rates
                            double old_trade_rate = total_trade_rate;
                            total_trade_rate *= epsilon_b;
                            total_rate += (total_delete_rate - old_trade_rate);
                            for (int i = 0; i < N; ++i) {
                                farms[i].b *= epsilon_b;
                                farms[i].tot_trade_rate *= epsilon_b;
                            }
                            for (int i = 0; i < trade_rates.size(); ++i) {
                                trade_rates[i] *= epsilon_b;
                            }
                        }
                    }
                }
            ...
            //reset farm values of b so they are not carried over to the next parameter value of epsilon_b
            if (trade_change_introduced == 1) {
                for (int i = 0; i < N; ++i) {
                    farms[i].b = farms[i].base_b;
                }
            }
