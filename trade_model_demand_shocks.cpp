...
int main() {
    ...
    out_file
            << "time,shock,eta,zeta,needs,supply,traders,trades,trade_size,in_flow,price,infected"
            << endl;

    //open file 2 and output column headers
    ofstream out_file2;
    out_file2.open(path + name2 + type);
    out_file2 << "shock,eta,zeta,needs,supply,traders,trades,trade_size,in_flow,price,infected"
              << endl;
    ...
    int num_params = 6; //number of parameters to run
    ...
    double demand_shock = 0; //size of shock to farm-level demand
    ...
    for (int np = 0; np < num_params; ++np) {
        ...
        demand_shock = np; //size of shock to demand
        ...
        for (int nr = 0; nr < num_reps; ++nr) {
            ...
            int shock_introduced = 0; //variable that takes value 1 when demand shocks are introduced
            ...
            while (dt < t_max) {
                ...
                //check to see if it's time to shock demand
                if (dt >= trade_change_time) {
                    if (shock_introduced == 0) {
                        shock_introduced = 1;
                        if (demand_shock != 0.0) {
                            total_demand += (demand_shock * N);
                            for (int i = 0; i < N; ++i) {
                                farms[i].demand += demand_shock;
                            }
                            //update partnership formation and trade rates
                            for (int i = 0; i < N; ++i) {
                                for (int j = 0; j < N; ++j) {
                                    if (i != j && farms[i].demand > 0.0 && farms[j].supply > 0.0) {
                                        if (find(farms[i].traders.begin(), farms[i].traders.end(), j) ==
                                            farms[i].traders.end()) {
                                            double old_add_rate;
                                            double new_add_rate;
                                            old_add_rate = farms[i].a * (farms[i].demand - demand_shock) *
                                                           pow(farms[j].supply, m) / (N - 1);
                                            new_add_rate =
                                                    farms[i].a * farms[i].demand * pow(farms[j].supply, m) / (N - 1);
                                            change_to_add_rate += (new_add_rate - old_add_rate);
                                            farms[i].tot_add_rate += (new_add_rate - old_add_rate);
                                            add_rates[farms[i].position_in_rate_vectors] += (new_add_rate -
                                                                                             old_add_rate);
                                        } else {
                                            double old_trade_rate;
                                            double new_trade_rate;
                                            old_trade_rate =
                                                    farms[i].b * min(farms[i].demand - demand_shock, farms[j].supply);
                                            new_trade_rate = farms[i].b * min(farms[i].demand, farms[j].supply);
                                            change_to_trade_rate += (new_trade_rate - old_trade_rate);
                                            farms[i].tot_trade_rate += (new_trade_rate - old_trade_rate);
                                            trade_rates[farms[i].position_in_rate_vectors] += (new_trade_rate -
                                                                                               old_trade_rate);
                                        }
                                    }
                                }
                            ...
                            for (int i = 0; i < N; ++i) {
                                double old_eta = farms[i].eta;
                                double new_eta = farms[i].eta_equi * pow(price / price_equi, -demand_elasticity);
                                farms[i].eta = new_eta;
                                change_to_demand_rate += (new_eta - old_eta);
                                demand_rates[farms[i].position_in_rate_vectors] += (new_eta - old_eta);

                                double old_zeta = farms[i].zeta;
                                double new_zeta = farms[i].zeta_equi * pow(price / price_equi, supply_elasticity);
                                farms[i].zeta = new_zeta;
                                change_to_supply_rate += (new_zeta - old_zeta);
                                supply_rates[farms[i].position_in_rate_vectors] += (new_zeta - old_zeta);

                            }
                            //update system total rates changed by shocks to demand
                            ...
                            total_rate += change_to_trade_rate + change_to_add_rate + change_to_demand_rate +
                            ...
                            if (time_index <= t_max / increment_size) {
                                ...
                                avg_demand_unit_time[time_index] += demand_shock;
                            }
                                ...
                                if (partial_rate >= event_rate && break_check != 1) {
                                    ...
                                    for (int j = 0; j < farms[it].traders.size() && break_check != 1; ++j) {
                                        ...
                                        if (partial_rate >= event_rate && break_check != 1) {
                                            ...
                                            //update supply and demand of buying and selling farm
                                            farms[it].demand -= trade_size;
                                            farms[farms[it].traders[j]].supply -= trade_size;
                                            ...
                                            if (farms[farms[it].traders[j]].disease_state == 1) {
                                                if (farms[it].disease_state == 0) {
                                                    B = 1 - pow(1 - inf_prob, trade_size); //calc prob batch infects buyer
                                                ...
                                                if (k != it) {
                                                    if (farms[k].supply > 0.0) {
                                                        ...
                                                        if (find(farms[it].traders.begin(), farms[it].traders.end(),
                                                            ...
                                                            double new_add_rate;
                                                            old_add_rate =
                                                                    farms[it].a * (demand_dummy + trade_size) *
                                            ...
                                            auto time_index = int(floor(dt / increment_size)) + 1;
                                            if (time_index <= t_max / increment_size) {
                                                trade_size_unit_time[time_index] += trade_size;
                                                in_vol_unit_time[time_index] += trade_size;
